...
它既可生成草稿，也可在目标明确且安全条件满足时维护云端日志。实时文档决定实际结构和位置；写作风格优先遵循当前要求、精确匹配后适用的已确认约定和代表性内容，通用模板仅作兜底。
...
独立记录使用 `lint_log_draft.py --profile full`（默认），章节内片段使用 `--profile append`；两者均允许自由组织正文。只有需要字段式模板提示时才使用 `--check-template`（仅 full），版式建议用 `--style-advice` 按需启用。事实、推测、决定与待确认是语义区别，不要求固定栏目、句数或表格列数。

离线输出保留 `ok`、`issues`，每项增加 `severity`。空白正文和明显凭据为 error，退出码 1；模板、版式、疑似个人信息和无法识别的日期等为 warning，退出码 0；参数或文件错误为 2。调用方不能继续把 `ok=true` 等同于所有检查无问题，应结合正文和来源审阅警告及内容充分性。每个显式日期与时区仍须核对实际有效性，但不强制改成统一显示格式。远端回执 warning 的结果未知规则不变。

每份文档可逐步形成自己的写作习惯。学习同章节的代表性内容，区分用户确认、暂定观察和一次性例外；不能把助手生成但未经认可的内容反复当作学习依据。八类示例展示不同组织方式，按文档需要采用，不是逐项填写的表单。
...
修改 `.local/` 需要当前明确要求，或已有针对该文档、仍有效的持续风格维护授权。后者仅允许在原有 Markdown 笔记中维护约定范围内的非敏感习惯、依据和修订，不允许修改信任标识或扩展目标。一次性写作要求不自动落盘；已有维护授权覆盖时无需每次再问。没有档案也能延续实时文档风格；升级不会替任何文档开启持续维护。详见[私有记忆合同](references/private-memory-contract.md)。
